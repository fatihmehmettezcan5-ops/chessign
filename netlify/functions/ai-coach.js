...
async function callNim(pgn, fen, question) {
...
async function callGemini(pgn, fen, question) {
...
exports.handler = async (event) => {
  ...
  const { pgn = '', fen = '', question = 'Analyze this game', history = [] } = body;

  let response, backend;
  try {
    response = await callNim(pgn, fen, question);
    backend = 'nvidia-nim';
  } catch (nimErr) {
    console.log('NVIDIA NIM failed, falling back to Gemini:', nimErr.message);
    try {
      response = await callGemini(pgn, fen, question);
      backend = 'gemini-pro';
    } catch (gemErr) {
      return { statusCode: 502, headers, body: JSON.stringify({ error: 'Both AI backends failed. NVIDIA: ' + nimErr.message + '; Gemini: ' + gemErr.message }) };
    }
  }

  return { statusCode: 200, headers, body: JSON.stringify({ response, backend }) };
};
